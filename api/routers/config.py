...
async def check_database_health() -> dict:
    ...
    """
    try:
        # No connection pooling (see open_notebook/database/repository.py) -
        # every call opens a fresh websocket, which on some Docker Desktop/
        # WSL2 setups consistently takes just over 2s to establish even
        # though the query itself is instant once connected. 2s was too
        # tight and produced false "offline" reports; 5s gives headroom
        # without meaningfully delaying a real outage's detection.
        result = await asyncio.wait_for(repo_query("RETURN 1"), timeout=5.0)
        if result:
            return {"status": "online"}
        return {"status": "offline", "error": "Empty result"}
    except asyncio.TimeoutError:
        logger.warning("Database health check timed out after 5 seconds")
