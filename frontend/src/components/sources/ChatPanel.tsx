            ...
            {messages.length === 0 ? (
              ...
              messages.map((message, index) => (
                <ChatMessage
                  ...
                  isCurrentlyStreaming={isStreaming && index === messages.length - 1 && message.type === 'ai'}
                />
              ))
            )}
            {/* Only show the generic "thinking" bubble before any AI content
                exists yet - once token deltas start arriving, the growing
                message bubble above (with its own streaming cursor) is the
                indicator, so this spinner shouldn't stack on top of it. */}
          ...
          {message.type === 'ai' ? (
            <>
              ...
              {isCurrentlyStreaming && (
                <span
                  aria-hidden="true"
                  className="inline-block w-1.5 h-4 ml-0.5 bg-current animate-pulse align-text-bottom"
                />
              )}
            </>
          ) : (
