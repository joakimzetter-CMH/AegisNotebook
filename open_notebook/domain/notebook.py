            ...
            if delete_exclusive_sources:
                ...
                for src in source_counts:
                    ...
                    if source_id and src.get("assigned_others", 0) == 0:
                        ...
                        unlinked_sources += 1
                # Clean up the artifact/reference relationships in one round
                # trip - db_connection() opens a fresh connection per repo_*
                # call (no pooling), so batching avoids paying that cost twice.
                await repo_query(
                    """
                ...
                # Count sources that will be unlinked, then clean up the
                # artifact/reference relationships - all in a single query on
                # a single connection. The SDK's query() only returns the
                # first statement's result, which is exactly the count we
                # need; the two DELETEs still run, their results just aren't
                # surfaced (verified separately - both execute regardless).
...
class Source(ObjectModel):
    ...
    fingerprint: Optional[str] = Field(
        default=None,
        description="Content fingerprint (file mtime+size, or a content hash "
        "for non-file sources), used to detect whether re-embedding is needed",
    )
    last_embedded_fingerprint: Optional[str] = Field(
        default=None,
        description="fingerprint value as of the last successful embed_source "
        "run — embed_source_command skips re-embedding when this still "
        "matches fingerprint",
    )
    ...
    async def delete(self) -> bool:
        ...
        # Batched into a single query (and therefore a single connection) -
        # db_connection() opens a fresh connection per repo_* call (no pooling),
        # so combining these two statements avoids paying that cost twice.
        try:
...
class ResearchSession(ObjectModel):
    """A session that can span MULTIPLE notebooks (via the `scopes` edge),
    letting one session mix cross-notebook Q&A with document generation.
    Deliberately separate from ChatSession/`refers_to`, which every current
    single-notebook chat consumer assumes is exactly one notebook - keeping
    that assumption intact rather than overloading it to be one-to-many.
    """

    table_name: ClassVar[str] = "research_session"
    nullable_fields: ClassVar[set[str]] = {"model_override"}
    title: Optional[str] = None
    model_override: Optional[str] = None

    async def scope_to_notebook(self, notebook_id: str) -> Any:
        if not notebook_id:
            raise InvalidInputError("Notebook ID must be provided")
        await Notebook.get(notebook_id)  # raises NotFoundError if invalid/missing
        return await self.relate("scopes", notebook_id)

    async def get_notebook_ids(self) -> List[str]:
        try:
            result = await repo_query(
                "SELECT VALUE out FROM scopes WHERE in = $id",
                {"id": ensure_record_id(self.id)},
            )
            return [str(notebook_id) for notebook_id in result] if result else []
        except Exception as e:
            logger.error(
                f"Error fetching notebooks for research session {self.id}: {str(e)}"
            )
            logger.exception(e)
            raise DatabaseOperationError(e)

    async def delete(self) -> bool:
        """Delete the session and its `scopes` edges. Generated documents
        produced by this session are kept (only `source_session` becomes a
        dangling reference) - they're a durable output the user may still
        want after cleaning up the session that produced them.
        """
        if self.id is None:
            raise InvalidInputError("Cannot delete object without an ID")
        try:
            await repo_query(
                "DELETE scopes WHERE in = $id", {"id": ensure_record_id(self.id)}
            )
        except Exception as e:
            logger.warning(
                f"Failed to delete scopes for research session {self.id}: {e}. "
                "Continuing with session deletion."
            )
        return await super().delete()


class GeneratedDocument(ObjectModel):
    """A multi-source document (e.g. a bilingual architecture doc) compiled
    from one or more notebooks in a ResearchSession. Distinct from Note
    (single-notebook-owned) and SourceInsight (tied to one source) - a
    generated document has no single owning notebook, so it links to its
    contributing notebooks via the many-to-many `compiled_from` edge,
    mirroring how sources link to notebooks via `reference`.
    """

    table_name: ClassVar[str] = "generated_document"
    nullable_fields: ClassVar[set[str]] = {"source_session", "embedding"}
    title: str
    content: str
    version: int = 1
    language: Literal["en", "sv", "bilingual"] = "bilingual"
    status: Literal["draft", "final"] = "draft"
    source_session: Optional[str] = None
    embedding: Optional[List[float]] = None

    async def save(self) -> Optional[str]:
        """Save the document and submit an embedding command (fire-and-forget),
        mirroring Note.save() so generated documents are retrievable by Ask.
        """
        await super().save()
        ...
        if self.id and self.content and self.content.strip():
            try:
                command_id = submit_command(
                    "open_notebook",
                    "embed_generated_document",
                    {"document_id": str(self.id)},
                )
                logger.debug(
                    f"Submitted embed_generated_document command {command_id} "
                    f"for {self.id}"
                )
                return command_id
            except Exception as e:
                logger.error(
                    f"Failed to submit embed_generated_document command for "
                    f"{self.id}: {e}"
                )
                return None
    ...
    def _prepare_save_data(self) -> dict:
        """Override to ensure source_session is RecordID format for the
        database - it's declared `option<record<research_session>>` (migration
        27), but the domain field is a plain str, same situation Source
        handles for its `command` field."""
        data = super()._prepare_save_data()
        if data.get("source_session") is not None:
            data["source_session"] = ensure_record_id(data["source_session"])
        return data

    async def compile_from_notebook(self, notebook_id: str) -> Any:
        if not notebook_id:
            raise InvalidInputError("Notebook ID must be provided")
        await Notebook.get(notebook_id)  # raises NotFoundError if invalid/missing
        # Refining a document re-links the same notebooks on every save, so
        # skip if the edge already exists (RELATE doesn't dedupe on its own).
        existing = await repo_query(
            "SELECT * FROM compiled_from WHERE in = $doc_id AND out = $notebook_id",
            {
        ...
        if existing:
            return existing
        return await self.relate("compiled_from", notebook_id)

    async def get_notebook_ids(self) -> List[str]:
        try:
            result = await repo_query(
                "SELECT VALUE out FROM compiled_from WHERE in = $id",
                {"id": ensure_record_id(self.id)},
            )
            return [str(notebook_id) for notebook_id in result] if result else []
        except Exception as e:
            logger.error(
                f"Error fetching notebooks for generated document {self.id}: {str(e)}"
            )
            logger.exception(e)
            raise DatabaseOperationError(e)
    ...
    async def delete(self) -> bool:
        ...
        try:
            await repo_query(
                "DELETE compiled_from WHERE in = $id",
                {"id": ensure_record_id(self.id)},
            )
        except Exception as e:
            logger.warning(
                f"Failed to delete compiled_from edges for generated document "
