...
export default function NotebookPage() {
  ...
  // useIsDesktop() starts as `false` until its effect runs (to avoid an SSR
  // hydration mismatch), so on the very first client render both the mobile
  // branch (`!isDesktop`) and the desktop branch (CSS-hidden but always
  // mounted) are active at once. That briefly mounts two separate ChatColumn
  // instances - two independent chat-session hooks - and when isDesktop
  // flips true a moment later, the mobile one unmounts, which reads as the
  // chat panel "reloading". Gate ChatColumn specifically on having mounted
  // client-side once, so exactly one instance ever exists.
