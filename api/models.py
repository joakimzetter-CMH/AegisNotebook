...
class AskRequest(NotebookScopeMixin):
    ...
    notebook_ids: Optional[List[str]] = Field(
        None,
        description="Restrict the search to these notebook IDs. Omit/empty for "
        "the default global search across every notebook.",
    )
...
class SkillCreate(BaseModel):
    ...
    title: str = Field(..., description="Display title for the skill")
    description: str = Field(..., description="Description of what this skill does")
    instructions: str = Field(
        ..., description="Operating instructions injected when this skill is matched"
    )
    enabled: bool = Field(True, description="Whether this skill can be retrieved")


class SkillUpdate(BaseModel):
    name: Optional[str] = Field(None, description="Skill name")
    title: Optional[str] = Field(None, description="Display title for the skill")
    description: Optional[str] = Field(
        None, description="Description of what this skill does"
    )
    instructions: Optional[str] = Field(
        None, description="Operating instructions injected when this skill is matched"
    )
    enabled: Optional[bool] = Field(None, description="Whether this skill can be retrieved")
...
class SkillResponse(BaseModel):
    ...
    knowledge_chunk_count: int = Field(
        0, description="Number of embedded knowledge chunks currently stored"
    )
    created: str
    updated: str


class SkillKnowledgeCreate(BaseModel):
    content: str = Field(..., description="Raw text to chunk, embed and attach")
    title: Optional[str] = Field(
        None, description="Label for this attachment, e.g. a filename"
    )


class SkillKnowledgeAttachResponse(BaseModel):
    command_id: str = Field(..., description="Background embedding job ID")
