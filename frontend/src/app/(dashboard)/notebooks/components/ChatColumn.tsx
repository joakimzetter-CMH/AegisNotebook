...
export function ChatColumn({ notebookId, contextSelections, sources, sourcesLoading }: ChatColumnProps) {
  ...
  // Show loading state only on this component's first-ever load, never again
  // for this mounted instance. Without this guard, any later background
  // refetch of sources/notes (window refocus, a mutation elsewhere
  // invalidating the same query, etc.) would blank the whole panel -
  // composer included - back to a bare spinner, which reads as the chat
