...
from open_notebook.domain.notebook import GeneratedDocument, Note, Source, SourceInsight
...
class EmbedGeneratedDocumentInput(CommandInput):
    """Input for embedding a single generated document."""

    document_id: str


class EmbedGeneratedDocumentOutput(CommandOutput):
    """Output from generated document embedding command."""
...
@command("embed_generated_document", app="open_notebook", retry=EMBED_RETRY_CONFIG)
async def embed_generated_document_command(
    input_data: EmbedGeneratedDocumentInput,
) -> EmbedGeneratedDocumentOutput:
    """
    Generate and store embedding for a single generated document.

    Mirrors embed_note_command: unified embedding pipeline with automatic
    chunking and mean pooling for documents that exceed the chunk size limit.
    ...
    async def embed() -> Tuple[Dict[str, Any], str]:
        return await _embed_markdown_record(
            ...
            record_id=input_data.document_id,
            loader=GeneratedDocument.get,
        )
    ...
    return EmbedGeneratedDocumentOutput(
        success=error_message is None,
...
def _compute_source_fingerprint(source: Source) -> str:
    """Cheap content fingerprint used to skip re-embedding unchanged sources.

    File-backed sources (upload) use mtime+size, mirroring the scheme Aegis
    used for its local knowledge-base cache — cheap to compute and good
    enough to detect a re-uploaded/re-processed file. Sources without a
    file on disk (link/text) fall back to a hash of the extracted text,
    since there's no filesystem timestamp to key off.
    """
...
async def embed_source_command(input_data: EmbedSourceInput) -> EmbedSourceOutput:
    ...
    async def embed() -> Tuple[Dict[str, Any], str]:
        ...
        # 1b. Skip re-embedding if content is unchanged since the last
        # successful embed (fingerprint cache, ported from Aegis's
        # manifest.json approach). Only trusted when there are existing
        # chunks to back it up, so a partially-cleaned-up source can't
        # report a false "already embedded" success.
        new_fingerprint = _compute_source_fingerprint(source)
        if (
            source.last_embedded_fingerprint
            and source.last_embedded_fingerprint == new_fingerprint
        ):
            existing_chunks = await source.get_embedded_chunks()
            if existing_chunks > 0:
                logger.info(
                    f"Source {input_data.source_id} unchanged since last embed "
                    f"(fingerprint match) - skipping re-embed"
                )
                return {"chunks_created": existing_chunks}, ": unchanged, skipped"
        ...
        # 7. Record the fingerprint this embed run is now consistent with,
        # so the next embed_source call for this source can skip if unchanged.
        source.fingerprint = new_fingerprint
        source.last_embedded_fingerprint = new_fingerprint
        await source.save()
