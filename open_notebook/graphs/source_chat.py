...
async def _empty_skill_context() -> str:
    return ""
...
def _call_model_with_source_context_inner(
    ...
    # Build source + skill context (run async code in new loop)
    def build_context():
        ...
        try:
            ...
            return new_loop.run_until_complete(
                asyncio.gather(
                    ...
                    build_skill_context(query) if query else _empty_skill_context(),
                )
            )
        finally:
            new_loop.close()
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            context_data, skill_context = future.result()
    except RuntimeError:
        # No event loop running, safe to create a new one
        context_data, skill_context = build_context()
