...
export interface StreamNotebookChatMessageRequest {
  message: string
  context: SendNotebookChatMessageRequest['context']
  model_override?: string
}
...
// Research session types (AegisNotebook - cross-notebook sessions that can
// mix Q&A and document generation, unlike the single-notebook chat above)
...
export interface ExecuteResearchSessionRequest {
  session_id: string
  message: string
  mode: 'ask' | 'generate'
...
export interface ExecuteResearchSessionResponse {
  session_id: string
  messages: ResearchSessionMessage[]
  active_document_id?: string | null
}

export interface StreamResearchSessionRequest {
  message: string
  mode: 'ask' | 'generate'
  language?: 'en' | 'sv' | 'bilingual'
  model_override?: string
...
export type ResearchSessionStreamEvent =
  | { type: 'user_message'; content: string; timestamp: string | null }
  | {
      type: 'status'
      stage:
  ...
  | { type: 'ai_message'; content: string; timestamp: string | null }
  | { type: 'complete'; active_document_id?: string | null }
  | { type: 'error'; message: string }
