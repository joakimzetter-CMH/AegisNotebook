...
Run AegisNotebook with a **separately installed Ollama** (not via Docker). This avoids Docker running the Ollama service while you use your own local Ollama installation.

## Prerequisites

1. **Docker Desktop** installed (for SurrealDB and AegisNotebook)
...
Create a new folder `AegisNotebook-external-ollama` and add these files:
...
## Step 3: Connect AegisNotebook to Host Ollama (1 min)

When AegisNotebook runs inside Docker, it cannot reach `localhost:11434` on your host directly. Use the special hostname:

| Host OS | Ollama URL in AegisNotebook |
|---------|----------------------------|
...
## Step 4: Start AegisNotebook (1 min)

Open a terminal in your `AegisNotebook-external-ollama` folder:
...
## Step 7: Access AegisNotebook (instant)
...
- **Add more models**: Run `ollama pull <model>`, then re-discover from AegisNotebook
