...
from open_notebook.domain.skill import skill_search
...
def format_skill_context(matches: list) -> str:
    """Render skill_search() results as the Markdown block injected into
    chat system prompts. Mirrors format_source_context's structure."""
    if not matches:
        return ""

    parts = ["## RELEVANT SKILLS"]
    for match in matches:
        parts.append(f"**Skill:** {match.get('title', 'Unknown')} [{match.get('parent_id')}]")
        content = match.get("matches")
...
async def build_skill_context(
    ...
    max_tokens: int = 4000,
    match_count: int = 3,
    min_similarity: float = SKILL_MIN_SIMILARITY,
) -> str:
    """Retrieve the skills most relevant to the latest user message and
    render them as a Markdown block, for injection into any chat's system
    prompt (notebook chat, source chat). Unlike build_notebook_context/
    build_source_context, this is not scoped to a notebook or source at
    all - skill_search() is always global.

    Returns "" (not raised) on any failure, or when nothing matches - a
    missing/misconfigured embedding model shouldn't break chat, it should
    just mean no skill gets injected.
    """
    if not query or not query.strip():
        return ""

    try:
        matches = await skill_search(
            query, match_count=match_count, min_similarity=min_similarity
        )
    except Exception as e:
        logger.warning(f"Skill retrieval failed, continuing without skills: {e}")
        return ""
    ...
    budget_used = 0
    for match in matches:
        rendered = format_skill_context([match])
    ...
    return format_skill_context(kept)
