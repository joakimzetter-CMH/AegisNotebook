...
Ollama provides free, local AI models that run on your own hardware. This guide covers everything you need to know about setting up Ollama with AegisNotebook, including different deployment scenarios and network configurations.
...
When both AegisNotebook and Ollama run directly on your machine:
...
### Scenario 2: AegisNotebook in Docker, Ollama on Host

When AegisNotebook runs in Docker but Ollama runs on your host machine:
...
When both AegisNotebook and Ollama run in the same Docker Compose stack:
...
This is the most common cause of "Failed to send message" errors. AegisNotebook requires the **exact model name** as it appears in Ollama.
...
**Step 2: Use the exact name when adding the model in AegisNotebook**
...
**Note:** Some models use `:latest` as the default tag. If `ollama list` shows `model:latest`, you must use `model:latest` in AegisNotebook, not just `model`.

**Step 3: Configure in AegisNotebook**
...
**1. "Ollama unavailable" in AegisNotebook**
...
```bash
# If AegisNotebook runs in Docker or on a different machine,
# Ollama must bind to all interfaces, not just localhost
export OLLAMA_HOST=0.0.0.0:11434
ollama serve
```
> **Why this is needed:** By default, Ollama only accepts connections from `localhost` (127.0.0.1). When AegisNotebook runs in Docker or on a different machine, it can't reach Ollama unless you configure `OLLAMA_HOST=0.0.0.0:11434` to accept external connections.
...
**From inside AegisNotebook container, test Ollama:**
```bash
# Get into container
...
1. **Model name mismatch**: The model name in AegisNotebook doesn't exactly match `ollama list`
2. **No default model configured**: You haven't set a default chat model in Settings → Models
3. **Model was deleted**: You removed the model from Ollama but didn't update AegisNotebook's defaults
4. **Model record deleted**: The model was removed from AegisNotebook but is still set as default
...
# Compare with what's configured in AegisNotebook
...
1. Refresh the AegisNotebook page
...
# AegisNotebook configuration (configure via Settings → API Keys instead)
...
- [AegisNotebook Discord](https://discord.gg/37XJPXfz2w) - Integration help
...
This comprehensive guide should help you successfully deploy and optimize Ollama with AegisNotebook. Start with the Quick Start section and refer to specific scenarios as needed.