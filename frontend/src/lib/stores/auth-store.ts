  ...
  persist(
    (set, get) => ({
      ...
      checkAuthRequired: async () => {
        // useAuth() composes its `isLoading` from this store's `isLoading`
        // OR'd with `!hasHydrated`. Without setting it here, that composed
        // value goes false the instant hydration finishes - before this
        // async call resolves - so the dashboard layout's redirect effect
        // (which fires on `!isLoading && !isAuthenticated`) sends a fresh
        // session to /login before we've even learned auth is disabled.
        set({ isLoading: true })
        try {
